...
SBSA_PATH=$TOP_DIR/edk2/ShellPkg/Application/sysarch-acs
...
build_sbsa_app()
{
    pushd $SBSA_PATH/apps/linux/sbsa-acs-app
    make clean
    make
    popd
}
...
build_pmu_app()
{
    pushd $SBSA_PATH/apps/linux/pmu_app
        arch=$(uname -m)
        echo $arch
        if [[ $arch = "aarch64" ]]
        then
...
build_mte_test()
{
    pushd $SBSA_PATH/apps/linux/mte
        arch=$(uname -m)
        echo "ARCH = $arch"
        if [[ $arch = "aarch64" ]]
        then
...
pack_in_ramdisk()
{
  ...
  cp $SBSA_PATH/apps/linux/sbsa-acs-app/sbsa $TOP_DIR/ramdisk/linux-sbsa
  cp -r $SBSA_PATH/apps/linux/pmu_app/pmuval $TOP_DIR/ramdisk/linux-sbsa

  #copy mte test to ramdisk
  cp $SBSA_PATH/apps/linux/mte/mte_test $TOP_DIR/ramdisk/linux-sbsa
